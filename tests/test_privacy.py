...
[class_id]).fetchone()[0]
        initial_contexts = db.execute("SELECT COUNT(*) FROM config_items WHERE item_type=? AND class_id = ?", [CONTEXT_ITEM_TYPE, class_id]).fetchone()[0]
    ...
    with app.app_context():
        ...
        contexts = db.execute("SELECT * FROM config_items WHERE item_type=? AND class_id = ?", [CONTEXT_ITEM_TYPE, class_id]).fetchall()
